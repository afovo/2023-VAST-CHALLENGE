...
const {anomalies} = toRefs(props)

let selected = props.data.hasOwnProperty(nodeID.value)?[props.data[nodeID.value]]:[]
let anomaly = [...anomalies.value].map(key => {
...
const schema = [
  ...
  { name: '1st neighbour political_organization', index: 5, text: 'political_organization' },
  ...
  { name: '1st neighbour in family_relationship', index: 14, text: 'family\nrelationship' },
  ...
  { name: '1st neighbour out family_relationship', index: 19, text: 'family\nrelationship' },
  ...
  { name: 'between neighbour family_relationship', index: 24, text: 'family\nrelationship' },
...
function init(){
  ...
  myChart.setOption({
    ...
    legend: {
      bottom: 30,
      data: ["Normal","Selected","Anomaly","Average"],
      itemGap: 20,
      textStyle: {
        color: '#fff',
        fontSize: 14
...
function neighborEdge(data) {
  ...
  var option = {
    ...
    legend: {
      top:0,
      data: [
        "membership", "partnership", "ownership", "family_relationship", "null"
      ]
    },
    series: [
      {
