...
with open('./Data/MC1.json', "r", encoding='utf-8') as r:
    data = json.load(r)
...
for node in data["nodes"]:
    if node['id'] not in node_id_mapping:
        node_names[current_id] = node['id']
        node_id_mapping[node['id']] = current_id
        current_id += 1
print(node_id_mapping.get(28379))
app = Flask(__name__)
...
data = loadmat('./Data/line_res.mat')
print(data)
simExperiment, algorithm = get_model_api(dataset=data)
...
def pick_node():
    ...
    picked_node_response = {"status": "success",
                            "data1": {"node": node_names[picked_node.id], "node_id": picked_node.id, "attribute": str(picked_node.contextFeatureVector)}}
...
def fetch_neighbors(node_id):
    ...
    return app.response_class(json.dumps(format_neighbor_feature_map(neighbor_feature_map)),
...
def fetch_anomalies():
    ...
    for key, value in anomalies.items():
        if value == 0:
            true_anomalies.append(node_names[key])
        else:
            false_anomalies.append(node_names[key])
