...
for i in range(9):
    a = np.divide(np.sum(parallelData_raw[i], axis=0), len(parallelData[node_types[i]])).tolist()
    a[-1] = "Average"
    parallelData[node_types[i]].update({"AVG": a})

json.dump(parallelData, open('parallelData1.json', 'w'))
