<template>
  <three-abila-map :path-coords-to-send="this.pathCoordsToSend" :range-to-abila="rangeToAbila"
    :color-map="colorMap"></three-abila-map>
  <a-select v-model="daySelected" style="position:absolute; top:79%;" placeholder="Please select a Day ..."
  ...
  <a-select v-model="rangeSelected" style="position:absolute; top:83%;" placeholder="Please select a Range Hour ..."
    @change="changeRange" id="timePickerS">
    <a-option :value="null" disabled> Select a Range Hour </a-option>
    <a-option v-for="(item, ind) in this.rangeH" :key="ind" :value=item>
      {{ item }}
    </a-option>
  </a-select>

  <!--  ToDo: 日期选取   参考 https://arco.design/vue/component/date-picker   范围看public/csv/df_gps_car-->
  <employee-list style="position:absolute; top:88%;" @get-employers="filterEmployers" @get-refresh="refreshMap"
    :employers_lst="this.employers_to_send"></employee-list>
